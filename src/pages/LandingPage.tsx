    ...
    <div className="min-h-screen bg-[#0b0f1a] text-white">
      ...
      <section className="relative overflow-hidden px-5 pt-24 pb-20 text-center">
        ...
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
          <div className="h-[500px] w-[800px] rounded-full bg-[hsl(217_91%_60%)/8%] blur-[120px]" />
        </div>
        ...
        <div className="relative mx-auto max-w-3xl">
          ...
          <h1 className="mb-5 text-4xl font-black leading-tight tracking-tight sm:text-5xl md:text-6xl">
            ...
            <span className="bg-gradient-to-r from-[hsl(217_91%_60%)] to-[hsl(217_91%_75%)] bg-clip-text text-transparent">
              comme un pro.
            </span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3">
            <Link
              to="/register"
              className="flex items-center gap-2 rounded-xl bg-[hsl(217_91%_60%)] px-6 py-3 text-sm font-semibold text-white hover:opacity-90 transition-opacity shadow-lg shadow-blue-500/20"
            >
        ...
        <div className="mx-auto max-w-6xl">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {FEATURES.map(({ icon: Icon, title, desc }) => (
              ...
              >
                <div className="mb-4 flex h-10 w-10 items-center justify-center rounded-xl bg-[hsl(217_91%_60%)/15%]">
                  <Icon size={18} className="text-[hsl(217_91%_70%)]" />
                </div>
      ...
      {/* How it works */}
      <section className="px-5 py-20 bg-white/[0.02]">
          ...
          <div className="grid gap-6 sm:grid-cols-3">
            {STEPS.map(({ num, title, desc }) => (
              <div key={num} className="text-center">
                <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-[hsl(217_91%_60%)/15%] border border-[hsl(217_91%_60%)/20%]">
                  <span className="text-lg font-black text-[hsl(217_91%_65%)]">{num}</span>
                </div>
                <h3 className="mb-2 text-sm font-semibold text-white">{title}</h3>
                <p className="text-sm text-white/50 leading-relaxed">{desc}</p>
              </div>
      ...
      <section className="px-5 py-20">
        <div className="mx-auto max-w-3xl">
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            {/* Pro */}
            <div className="rounded-2xl border border-[hsl(217_91%_60%)/40%] bg-[hsl(217_91%_60%)/8%] p-6 relative overflow-hidden">
              <div className="absolute top-4 right-4">
                <span className="rounded-full bg-[hsl(217_91%_60%)] px-2.5 py-0.5 text-[10px] font-bold text-white uppercase tracking-wide">
                  Recommandé
                </span>
              </div>
              <p className="text-xs font-semibold uppercase tracking-wider text-[hsl(217_91%_70%)] mb-3">Pro</p>
              ...
              <ul className="space-y-2.5 mb-6">
                {['Prospects illimités', 'Import CSV / Excel', 'Email direct aux prospects', 'Toutes les fonctionnalités Free', 'Export Excel/CSV', 'Support prioritaire'].map(f => (
                  <li key={f} className="flex items-center gap-2 text-sm text-white/80">
                    <CheckCircle2 size={13} className="text-[hsl(217_91%_65%)] flex-shrink-0" />
                    {f}
                  </li>
                ))}
              </ul>
              <Link
                to="/register"
                className="flex items-center justify-center gap-2 w-full rounded-xl bg-[hsl(217_91%_60%)] py-2.5 text-sm font-semibold text-white hover:opacity-90 transition-opacity shadow-lg shadow-blue-500/20"
              >
                <Zap size={14} />
      ...
      <section className="px-5 py-24 text-center relative overflow-hidden">
        <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
          <div className="h-[400px] w-[600px] rounded-full bg-[hsl(217_91%_60%)/6%] blur-[100px]" />
        </div>
        <div className="relative mx-auto max-w-xl">
          ...
          <Link
            to="/register"
            className="inline-flex items-center gap-2 rounded-xl bg-[hsl(217_91%_60%)] px-8 py-3.5 text-sm font-semibold text-white hover:opacity-90 transition-opacity shadow-lg shadow-blue-500/20"
          >
